...
`generate`는 백엔드가 둘이다. **Claude Code(`claude` 명령)가 이미 깔려 있으면 API 키 없이
그대로 돌아간다.**

```bash
semester-board generate --backend claude-cli   # 설치된 claude 명령 사용
semester-board generate --backend api          # ANTHROPIC_API_KEY 사용
```

지정하지 않으면 되는 쪽을 자동으로 고른다. 차이는 스키마를 누가 강제하느냐뿐이다 —
`api`는 서버가 JSON 스키마를 보장하고, `claude-cli`는 프롬프트로 요구한 뒤 받은 결과를
직접 검사한다.

**실측 비용** — 딥러닝응용 한 과목(강의계획서 6쪽 + 슬라이드 78쪽·65쪽 + 녹음 전사)을
`claude-cli`로 처음부터 만들었을 때 **호출 4회 · 3분 43초 · $1.88**이었다. 강의계획서
뼈대만 다시 만드는 것은 1회 $0.22다. 같은 파일은 내용 해시로 캐시되므로 반복 실행은 공짜다.

`generate`는 같은 파일을 두 번 보내지 않는다. 내용 해시로 캐시하므로, 자료를 고쳐
올렸을 때만 다시 만든다. `--dry-run`으로 무엇이 호출될지 먼저 확인할 수 있다.
...
**모델이 규칙을 어길 것을 전제한다.** 학수번호·담당·교재·주차 제목은 화면에서 escape되어
나가는 자리라, 태그가 섞이면 `<code>TBA</code>`가 글자 그대로 보인다. 프롬프트로 금지하고
받은 뒤에 한 번 더 지운다.
